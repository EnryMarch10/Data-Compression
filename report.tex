...
Martin-Löf extended the definition for random elements with three approaches (\cite{AlgorithmicRandomnessAndComplexity}) to the
definition of algorithmic randomness for infinite sequences:
...
Despite the definitions he real improvement of Martin-Löf was the introduction of tests for randomness
(\cite{TheDefinitionOfRandomSequences}).

\begin{example}
Let's consider a long binary sequence "000...0" of all zeros.
It can be said that this sequence does not converge to 1/2 in the probability of 1 and 0, so it is clearly non-random.
But the reverse is not true: For instance "101101101101101...101" is not random because "101" occurs too often.
Moreover not random sequences like the digits of \(\pi\), cause the most difficulties.
...
object.} (\cite{AlgorithmicInformationTheoryBriefGuide}) formally defines what is randomness of individual objects
(binary strings).
...
For details on these implementations and their examples, see \cite{TheDataCompressionBook2ndEdition} and
...
This factor is not so indicative like the others but some tests has been yet done.
In \cite{OnTheRandomnessOfCompressedData} it is stated that surprisingly, the best compression schemes do not have the most random
output.
